...
**1) Few-step generation.** In the image & video domain, continuous diffusion maps different noise points to different data samples, making one-step generation possible through learning this many-to-many mapping. Masked diffusion, however, has its prior distribution collapsed into one single fully masked state. One-step generation based on this one-to-many mapping will be then converged to one-to-one mapping, which severely hurts the sample diversity [9]. Furthermore, few-step generation of masked diffusion suffers from *parallel dilemma* [10], which mixes up different modes due to the variety of targets. As shown in the figure below, this phenomenon will degrade the sample quality.
...
**2) Controllability**. Image and video diffusion can be semantically controlled by manipulating continuous latents. For example, classifier guidance uses the gradient of classifiers to shift generation trajectories towards specific types of data [11]. By contrast, masked diffusion only supports token-level editing like clozing.
...
Frontiers in diffusion language models have took actions on this question in 2025: They traced back to architectures with dispersed priors, trying to get some losed diffusion characteristics back. One well-known try is DUO [12], an improved version of uniform-state discrete diffusion. Although it underperforms masked diffusion under language modeling benchmarks like OpenWebText, DUO maintains better sample quality after distillation for few-step generation while adopting the guidance mechanism [13] designed for uniform-state discrete diffusion. A more recent study [14] showed that DUO beat both masked diffusion and autoregressive in a scaled-up setting on GSM8K, a widely-used math benchmark. This result challenges the trend of limitlessly making diffusion look like autoregressive.
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/langflow/pipe.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
Flow Matching (FM) [15] constructs a velocity field $$\mathbf{u}_t(\mathbf{z}_t)$$ that transports a Gaussian prior $$p_\text{prior} = \mathcal{N}(\mathbf{0}, \mathbf{I})$$ to the data distribution $$p_\text{data}$$ by solving an ODE $$d\mathbf{z}_t = \mathbf{u}_t(\mathbf{z}_t)\, dt$$. In practice, we use an affine Gaussian probability path:
...
LangFlow adopts a deterministic generative process via probability flow ODE. We therefore derive a likelihood bound tailored specifically to this ODE trajectory, following Flow Matching [15]. For a sequence $$\mathbf{x} = (x^{(1)}, \ldots, x^{(L)})$$ with embedding dimension $$D$$, the log-likelihood of LangFlow satisfies the following evidence lower bound:
...
[5] Arriola, M., Sahoo, S., Schiff, Y., Gokaslan, A., Kuleshov, V. "Block diffusion: Interpolating between autoregressive and diffusion language models." 2025.

[6] *Placeholder for reference [6].*

[7] *Placeholder for reference [7].*

[8] *Placeholder for reference [8].*

[9] *Placeholder for reference [9].*

[10] *Placeholder for reference [10].*

[11] Dhariwal, P. and Nichol, A. "Diffusion models beat GANs on image synthesis." *NeurIPS*, 2021.

[12] *Placeholder for reference [12] (DUO).*

[13] *Placeholder for reference [13].*

[14] *Placeholder for reference [14].*

[15] Lipman, Y., Chen, R. T., Ben-Hamu, H., Nickel, M. "Flow matching for generative modeling." *ICLR*, 2023.

[16] *Placeholder for reference [16] (VFM).*

[17] Peebles, W. and Xie, S. "Scalable diffusion models with transformers." *ICCV*, 2023.

[18] Chen, T., Zhang, R., and Hinton, G. "Analog bits: Generating discrete data using diffusion models with self-conditioning." *ICLR*, 2023.

[19] *Placeholder for reference [19].*

[20] *Placeholder for reference [20].*
